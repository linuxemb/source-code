...
const long Request::getKeepAlive()const
{
	HttpMap::const_iterator it;
	...
	if(it!=httpHeader.end()){
		if(it->second=="keep-alive"){
			it=httpHeader.find("Keep-Alive");
			if(it!=httpHeader.end()) return atol(it->second.c_str());
			return HttpServer::defaultKeepAlive;
		}
	}
	return 0;
}
